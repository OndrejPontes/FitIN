...
public class GymFiltering {
    ...
    public static List<GymPreview> filterGymsPreviews(Filter f, List<Gym> gyms, Integer best) {
        ...
        if (filter != null) {
            ...
            if (filter.getEquipments() != null && filter.getEquipments().size() != 0)
                filterByEquipments();
            if (filter.getActivities() != null && filter.getActivities().size() != 0)
                filterByActivities();
        }
        if (best != null) {
    ...
    private static void filterByLocation() {
        ...
        for (Gym gym : help) {
            if (gym.getCoordinates().getLatitude() < c1.getLatitude() ||
                    ...
                    gym.getCoordinates().getLongitude() > c2.getLongitude())
                filteredGym.remove(gym);
        }
    }
